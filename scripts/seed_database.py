...
class DatabaseSeeder:
    """Seed Supabase database with generated dummy data"""
    ...
    def seed_borrowers(self):
        ...
        borrowers = self.load_json_file('borrowers_seed.json')

        if not borrowers:
            logger.warning("No borrower data found")
            return

        # Remove 'id' field as Supabase will generate UUIDs
    ...
    def seed_loans(self):
        ...
        loans = self.load_json_file('loans_seed.json')

        if not loans:
            logger.warning("No loan data found")
            return

        # Get borrower ID mappings (id -> UUID)
        borrowers_response = self.client.table('borrowers').select('id, full_name').execute()
        borrowers = borrowers_response.data

        # Create mapping from sequential ID to UUID
        # Assumes borrowers were seeded in order
        id_mapping = {i + 1: borrower['id'] for i, borrower in enumerate(borrowers)}

        # Update borrower_id references
        for loan in loans:
            old_id = loan.pop('id', None)
            borrower_seq_id = loan['borrower_id']
            loan['borrower_id'] = id_mapping.get(borrower_seq_id)
            loan.pop('repayment_profile', None)  # Remove non-DB field
    ...
    def seed_repayments(self):
        ...
        repayments = self.load_json_file('repayments_seed.json')
        ...
        # Create mapping (assuming loans were seeded in order)
        loan_id_mapping = {i + 1: loan['id'] for i, loan in enumerate(loans)}

        # Update loan_id references
        for repayment in repayments:
            repayment.pop('id', None)
            loan_seq_id = repayment['loan_id']
    ...
    def seed_photos(self):
        ...
        photos = self.load_json_file('photos_seed.json')
        ...
        for photo in photos:
            photo.pop('id', None)
            borrower_seq_id = photo['borrower_id']
    ...
    def seed_field_notes(self):
        ...
        field_notes = self.load_json_file('field_notes_seed.json')
        ...
        for note in field_notes:
            note.pop('id', None)
            borrower_seq_id = note['borrower_id']
    ...
    def clear_all_data(self):
        """Clear all data from tables (DANGEROUS - use with caution)"""
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Seed Amara AI database with dummy data')
