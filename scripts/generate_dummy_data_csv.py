...
class DummyDataGeneratorCSV:
    """Generate realistic dummy data for Amara AI MVP in CSV format"""
    ...
    def generate_borrowers(self):
        ...
        for i, business in enumerate(business_distribution):
            borrower = {
                ...
                "keeps_financial_records": random.choice([True, False]),
    ...
    def generate_field_notes(self):
        """Generate field agent narratives"""
        ...
        for borrower in self.borrowers:
            ...
            for _ in range(num_notes):
                ...
                note_text = self._generate_realistic_note(borrower)

                note = {
                    "id": len(self.field_notes) + 1,
                    "borrower_id": borrower["id"],
                    "loan_id": "",  # Empty for CSV
                    "note_type": note_type,
    ...
    def _generate_realistic_note(self, borrower):
        """Generate contextual field note narratives"""
        name = borrower["full_name"].split()[1]
        business = borrower["business_type"]
        income = borrower["claimed_monthly_income"]
        daily_income = income / 30

        templates = [
            f"Kunjungan ke usaha {business} milik {borrower['full_name']}. Usaha terletak di {borrower['village']} {borrower['district']}. Ibu {name} menceritakan penghasilan sekitar Rp {income:,.0f} per bulan atau sekitar Rp {daily_income:,.0f} per hari. Sudah menjalankan usaha selama {borrower['years_in_business']} tahun. {'Sudah punya buku catatan sederhana' if borrower['keeps_financial_records'] else 'Belum punya catatan keuangan yang rapi'}. Sikap kooperatif dan terbuka menceritakan kondisi usaha.",

            f"Observasi bisnis {business}. Usaha terlihat {'ramai dengan pelanggan tetap' if random.random() < 0.6 else 'sepi tapi stabil'}. Ibu {name} menjelaskan omzet harian sekitar Rp {daily_income:,.0f} tergantung hari. Modal kerja harian sekitar Rp {income * 0.4:,.0f} untuk beli bahan baku dan stok. {'Tempat usaha milik sendiri' if random.random() < 0.7 else 'Sewa tempat bulanan'}. Peralatan usaha {'dalam kondisi baik' if random.random() < 0.6 else 'ada yang perlu diganti'}.",

            f"Pertemuan dengan Ibu {name} di rumah yang juga jadi tempat usaha {business}. Rumah {'sederhana ukuran 6x8 meter dinding tembok setengah bata' if random.random() < 0.6 else 'cukup layak dengan kondisi terawat'}. Usaha sudah berjalan {borrower['years_in_business']} tahun dengan pelanggan {'tetap dan loyal' if random.random() < 0.5 else 'yang lumayan stabil'}. Pendidikan {borrower['education_level']} {'bisa baca tulis dan hitung dengan baik' if borrower['education_level'] != 'SD' else 'bisa baca tulis dasar'}. {'Transparan dan kooperatif' if random.random() < 0.7 else 'Agak tertutup tapi mau bekerja sama'} saat wawancara."
        ]

        return random.choice(templates)

    def save_to_csv(self, output_dir="data/seed"):
        ...
        # Save borrowers
        with open(f"{output_dir}/borrowers_seed.csv", "w", newline='', encoding='utf-8') as f:
            if self.borrowers:
                writer = csv.DictWriter(f, fieldnames=self.borrowers[0].keys())
                writer.writeheader()
                writer.writerows(self.borrowers)

        # Save loans
        with open(f"{output_dir}/loans_seed.csv", "w", newline='', encoding='utf-8') as f:
            if self.loans:
                writer = csv.DictWriter(f, fieldnames=self.loans[0].keys())
                writer.writeheader()
                writer.writerows(self.loans)

        # Save repayments
        with open(f"{output_dir}/repayments_seed.csv", "w", newline='', encoding='utf-8') as f:
            if self.repayments:
                writer = csv.DictWriter(f, fieldnames=self.repayments[0].keys())
                writer.writeheader()
                writer.writerows(self.repayments)

        # Save photos
        with open(f"{output_dir}/photos_seed.csv", "w", newline='', encoding='utf-8') as f:
            if self.photos:
                writer = csv.DictWriter(f, fieldnames=self.photos[0].keys())
                writer.writeheader()
                writer.writerows(self.photos)

        # Save field notes
        with open(f"{output_dir}/field_notes_seed.csv", "w", newline='', encoding='utf-8') as f:
            if self.field_notes:
                writer = csv.DictWriter(f, fieldnames=self.field_notes[0].keys())
                writer.writeheader()
                writer.writerows(self.field_notes)
...
def main():
    """Main execution"""
    print("=" * 60)
    print("🎯 AMARA AI - CSV DATA GENERATOR")
    print("=" * 60)

    generator = DummyDataGeneratorCSV(num_borrowers=75)
    ...
    print("=" * 60)
    print(f"Borrowers: {len(generator.borrowers)}")
    print(f"Loans: {len(generator.loans)}")
    print(f"Repayments: {len(generator.repayments)}")
    print(f"Photos: {len(generator.photos)}")
    print(f"Field Notes: {len(generator.field_notes)}")
    print("=" * 60)
    print("✨ CSV data generation complete!")
    print("\nFiles saved in: data/seed/")
    print("  - borrowers_seed.csv")
    print("  - loans_seed.csv")
    print("  - repayments_seed.csv")
    print("  - photos_seed.csv")
    print("  - field_notes_seed.csv")
